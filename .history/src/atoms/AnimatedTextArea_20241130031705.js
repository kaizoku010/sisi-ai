...
function AnimatedTextArea() {
  ...
  const getHighlightedText = () => {
    ...
    highlightWords.forEach((highlight, index) => {
      ...
      if (highlightIndex >= 0) {
        // Debugging: Check slicing process
        console.log("Highlight Found:", highlight.word, "at index", highlightIndex);
        ...
        if (highlightIndex > 0) {
          ...
          console.log("Before Highlight:", beforeHighlight);
        }
    ...
    // Add any remaining text after processing all highlights
    if (remainingText) {
      ...
      if (part.isHighlighted) {
        return (
          <motion.span
            ...
            initial={{ opacity: 0 }}
            animate={{
              opacity: part.isActive ? 1 : 0, // Fade background
